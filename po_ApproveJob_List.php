...
<?php
                $conn = mysqli_connect("localhost", "root", "", "minor_project");
                  $query="select * from jobdetails where joba_status=1 and status=1";
                    $count = 0;
                    $records = mysqli_query($conn, $query);
                    mysqli_close($conn);
?>
<!-- Page Heading -->
<h1 class="h3 mb-4 text-gray-800">APPROVE JOB LIST</h1>
<div class="d-flex justify-content-end box" style="position:relative; bottom:-10px; left:-10px;  z-index: 9; ">
    <button class="btn btn-danger" >
        <a class="text-white" href='approvejoblistpdf.php'><i class="fas fa-file-pdf text-white"></i>&nbsp;PDF</a>
    </button>
</div>
